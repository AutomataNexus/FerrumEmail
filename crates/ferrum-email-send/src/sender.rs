...
impl Sender {
    ...
    pub async fn send_message(&self, message: EmailMessage) -> Result<SendResult, EmailError> {
        ...
        if message.subject.len() > 998 {
            return Err(EmailError::MissingField(
                "Subject exceeds 998 character limit".into(),
            ));
        }
        ...
        if message.html.len() > 10_485_760 {
            return Err(EmailError::MissingField(
                "HTML body exceeds 10MB limit".into(),
            ));
        }
        let total_attachment_size: usize =
            message.attachments.iter().map(|a| a.content.len()).sum();
        if total_attachment_size > 25_165_824 {
            return Err(EmailError::MissingField(
                "Attachments exceed 24MB limit".into(),
            ));
        }
